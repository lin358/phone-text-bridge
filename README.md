...
使用时，先把电脑光标放到要输入的文字框里，再从手机页面发送文本。电脑端会复制文本并按 `Ctrl+V`，只要这个粘贴动作执行成功，手机文本框就会自动清空。
...
- 当前版本以“电脑端已经执行粘贴动作”为成功标准。它无法百分百知道目标软件是否真的接收了文字，所以使用前仍要确认电脑光标位置。
...
After the PC executes the paste action, the phone text is cleared automatically.
